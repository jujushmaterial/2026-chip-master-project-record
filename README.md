...
- `members/jujushmaterial/`은 `jujushmaterial`만 생성·수정·이동·이름 변경·삭제할 수 있다.
- `members/ryu980920/`은 `ryu980920`만 생성·수정·이동·이름 변경·삭제할 수 있다.
- 각 구성원과 해당 구성원이 사용하는 AI는 동료의 개인 폴더를 **읽기 전용**으로만 확인한다.
- 동료의 개인 폴더에서는 파일이나 폴더를 새로 만들거나, 기존 내용을 수정·이동·이름 변경·삭제하지 않는다.
- 동료 기록에서 오류, 누락 또는 수정 필요 사항을 발견하면 직접 고치지 않고 해당 구성원에게 알린다.
- AI가 레포 전체에 대한 쓰기 권한을 가지고 있더라도 동료 개인 폴더에는 쓰기 작업을 수행하지 않는다.

### Shared Area

- `README.md`, `shared/`, `templates/`는 공용 영역이다.
- 공용 영역을 변경한 경우 변경 목적과 내용을 상대 구성원에게 공유한다.
- 대규모 폴더 이동, 이름 변경 또는 공통 규칙 변경은 먼저 협의한다.
- 중요한 원본 데이터는 덮어쓰기보다 새 버전으로 저장한다.
- 병합 충돌이 발생하면 한쪽 기록을 임의로 삭제하지 않는다.
- `main` 브랜치에 강제 푸시하지 않는다.
